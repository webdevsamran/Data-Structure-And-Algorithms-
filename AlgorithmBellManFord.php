...
class Graph
{
    ...
    public function BellManFordAlgorithm($u): void
    {
        $V = $this->V;
        $E = $this->E;
        ...
        for ($i = 1; $i <= $V; $i++) {
            for ($j = 0; $j < $E; $j++) {
                ...
                $v = $this->edge[$j]->v;
                $w = $this->edge[$j]->w;
                if ($distance[$u] != PHP_INT_MAX && $distance[$v] > $distance[$u] + $w) {
        ...
        for ($i = 0; $i < $E; $i++) {
            ...
            $w = $this->edge[$i]->w;
            if ($distance[$u] != PHP_INT_MAX && $distance[$v] > $distance[$u] + $w) {
                echo "Graph Contains a Cycle<br/>\n";
                return;
            }
        }
        $this->printBMF($distance, $V);
        return;
    }

    public function printBMF(&$distance, $V): void
    {
